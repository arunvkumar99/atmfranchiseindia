...
async function getIncompleteTranslations(): Promise<Array<{
  ...
  const { data, error } = await supabase
    ...
    .select('id, original_text, translated_text, language_code, page_path, content_key')
...
async function safeUpdateTranslation(id: string, translatedText: string): Promise<boolean> {
  try {
...
Deno.serve(async (req) => {
  console.log(`📥 Fix translation completion request: ${req.method} ${req.url}`);
  ...
  try {
    console.log('🚀 Starting FIXED translation completion process...');
    ...
    console.log(`🎯 Processing ${incompleteTranslations.length} incomplete translations`);
      ...
      for (const item of items) {
        try {
          ...
          if (item.original_text.includes('/lovable-uploads/') || 
            ...
            const success = await safeUpdateTranslation(item.id, item.original_text);
            if (success) {
              completed++;
          ...
          const success = await safeUpdateTranslation(item.id, translatedText);
    ...
    console.log(`🎉 FIXED translation completion finished!`);
    console.log(`📊 Final Stats: Processed: ${processed}, Completed: ${completed}, Failed: ${failed}`);
    ...
    console.error('❌ FIXED translation completion error:', error);
    
    return new Response(
      JSON.stringify({
        success: false,
        error: error.message,
        message: 'Failed to complete translations with fixed function'
      }),
      {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 500,
