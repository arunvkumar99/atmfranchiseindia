...
export class ErrorLogger {
  ...
  logError(error: Error, context: Partial<ErrorContext> = {}, severity: ErrorSeverity = ErrorSeverity.MEDIUM): void {
    ...
    switch (severity) {
      ...
      case ErrorSeverity.HIGH:
        if (import.meta.env?.DEV) if (import.meta.env.DEV) { console.error('🚨 ERROR:', formattedError); }
        break;
      case ErrorSeverity.CRITICAL:
        if (import.meta.env?.DEV) if (import.meta.env.DEV) { console.error('💀 CRITICAL:', formattedError); }
        break;
    }
  ...
  private async sendToMonitoring(error: Error, context: ErrorContext, severity: ErrorSeverity): Promise<void> {
    try {
      ...
      if (import.meta.env?.DEV) if (import.meta.env.DEV) { console.error('Failed to send error to monitoring:', monitoringError); }
    }
  }
}
